...
def export_ner(sess, model, FLAGS, ner_test_batcher, string_int_maps, out_file):
	'''
	Export ner labels for each token
	'''
	...
	with open('%s_ner.txt' %out_file, 'w') as f:
		while ner_test_batcher.epoch == start_epoch:
			...
			labels, predictions = zip(*filtered)
			tokens = np.reshape(toks, [-1])
			for l, p, t in zip(labels, predictions, tokens):
				f.write('%s\t%s\t%s\n' %(labels_id_str_map[l], labels_id_str_map[p], token_id_str_map[t]))

def export_relations(sess, model, FLAGS, positive_test_batcher, negative_test_batcher,
	...
	with open('%s_eps.txt' %out_file, 'w') as f:
