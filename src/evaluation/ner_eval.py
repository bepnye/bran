...
def ner_eval(ner_test_batcher, sess, model, FLAGS, string_int_maps):
	...
	def class_label(l):
		return 'O' if l in ['0', 'O', '<PAD>', '<START>', '<END>'] else l.split('-')[1]
	labels_id_class_map = { i: class_label(labels_id_str_map[i]) for i in labels_id_str_map }
	classes = set(labels_id_class_map.values())
	labels_class_int_map = { c: idx for idx, c in enumerate(classes) }
	labels_id_int_map = { i: labels_class_int_map[labels_id_class_map[i]] for i in labels_id_str_map }
	...
	while ner_test_batcher.epoch == start_epoch:
		...
		for p, l, m in zip(max_preds, flat_labels, mask):
			if m == 1:
				confusion[labels_id_int_map[p], labels_id_int_map[l]] += 1

	return compute_f1(confusion, labels_class_int_map, 0, labels_class_int_map['O'])
